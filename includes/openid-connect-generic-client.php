...
class OpenID_Connect_Generic_Client {
	...
	// JPH Output variable state to browser console for debugging
	...
	// JPH - Include Base64 encoded Header for CLIENT_SECRET_BASIC
	...
	function request_new_tokens( $refresh_token ) {
        // JPH updated code to move auth data to header
      	         $request = array(
                        'body' => array(
	...
	function validate_user_claim( $user_claim, $id_token_claim ) {
		...
		if ( ! is_array( $user_claim ) ){
			//console_log($id_token_claim);
			//console_log($user_claim);
			return new WP_Error( 'invalid-user-claim', __( 'Invalid user claim(see browser console log for debug)' ), $user_claim );
		}
